...
## 최근 개선 사항 (Release v4.1.0)

### 1. 경량 NLU 구조 개선 및 일관화

- 프론트엔드와 백엔드 모두에 경량 NLU(Natural Language Understanding) 구조를 구현했습니다.
- 사용자 질의를 인텐트(의도)와 엔티티(객체)로 일관되게 분리하여 처리합니다.
- 인텐트: CPU_STATUS, MEMORY_STATUS, DISK_STATUS, INCIDENT_QUERY, SERVER_STATUS 등
- 엔티티: server_type, threshold, time_range, severity 등

### 2. 프론트엔드-백엔드 질의 처리 로직 개선

- 프론트엔드 `process_query.js`에 백엔드와 호환되는 NLU 분석 기능을 추가했습니다.
- 백엔드 API 응답 구조를 개선하여 인텐트와 엔티티 정보를 명시적으로 프론트엔드에 제공합니다.
- 오류 처리를 강화하여 모든 API 엔드포인트에 일관된 에러 핸들링을 적용했습니다.

### 3. 컨텍스트 처리 방식 개선

- 대화 컨텍스트를 활용하여 후속 질문에 더 정확하고 맥락에 맞는 응답을 제공합니다.
- 예: 특정 서버에 대한 질문 후 "왜 그런지 설명해줘"와 같은 후속 질문에 컨텍스트를 유지합니다.
- 이전 대화에서 언급된 메트릭 타입(CPU, 메모리 등)을 기억하고 후속 질문에 활용합니다.

### 4. 개발자 경험 개선

- API 응답 구조를 일관되게 유지하여 프론트엔드 개발이 더 쉬워졌습니다.
- 모든 API에 오류 처리 로직을 추가하여 안정성을 향상시켰습니다.
- 백엔드 코드 구조를 개선하여 유지보수성이 향상되었습니다.

## 향후 개선 계획

1. 컨텍스트 파일 구조화 - 현재 텍스트 파일 기반 컨텍스트를 JSON/YAML 형식으로 변경
2. NLU 기능 강화 - 더 다양한 인텐트 및 엔티티 인식 추가
3. 프론트엔드 UI 개선 - 컨텍스트 인식 기능을 시각적으로 표현
4. 백엔드 성능 최적화 - 대규모 메트릭 데이터 처리 성능 개선 